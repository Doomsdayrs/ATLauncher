    ...
    public Observable<Boolean> getHasUpdateObservable(Instance instance) {
        if (instance.launcher.vanillaInstance) {
            ...
            if (instance.isModpacksChPack()) {
                return ModpacksChUpdateManager.getSubject(instance).map(latestVersion ->
                ...
                return CurseForgeUpdateManager.getSubject(instance).map(latestVersion ->
                ...
                if (instance.isTechnicSolderPack()) {
                    return TechnicModpackUpdateManager.getSolderSubject(instance).map(latestVersion ->
                        latestVersion.isPresent() && !latestVersion.get().latest.equals(instance.launcher.version)
                    );
                } else {
                    return TechnicModpackUpdateManager.getSubject(instance).map(latestVersion ->
                        latestVersion.isPresent() && !latestVersion.get().version.equals(instance.launcher.version)
                    );
                }
            } else if (instance.isModrinthPack()) {
                return ModrinthModpackUpdateManager.getSubject(instance).map(latestVersion ->
